...
class BaseTask:
    ...
    def _generate_experience_one(
        ...
        if hasattr(client, "_disturb_mgr") and getattr(getattr(client, "_disturb_mgr"), "reset_call_counts", None):
            try:
                client._disturb_mgr.reset_call_counts()
            except Exception:
                pass
        reward = 0.0
        done = False
        ...
        while not done:
            if isinstance(agent, Agent):
                ...
                if not generated_text.strip():
                    print("[APIAgent] empty response, abort this episode.")
                    break
                conversation.append(
        ...
        if hasattr(client, "_disturb_mgr") and getattr(getattr(client, "_disturb_mgr"), "disturb", None) in (3, 4):
            disturb0_calls = int(getattr(client._disturb_mgr, "disturb0_calls", 0))
            disturb1_calls = int(getattr(client._disturb_mgr, "disturb1_calls", 0))
        ...
        if isinstance(agent, Agent):
            return ExperienceOutput(
                ...
                disturb0_calls=disturb0_calls,
                disturb1_calls=disturb1_calls,
            )
        elif isinstance(agent, APIAgent):
