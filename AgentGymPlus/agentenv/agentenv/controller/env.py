...
class BaseEnvClient(metaclass=ABCMeta):
    ...
    def __init__(self, action_format: ActionFormat = "react", disturb: int = 0) -> None:
        """Base class for environment clients.

        Args:
            action_format: How the agent should express actions.
            disturb: Interface disturbance mode.
                - 0: no disturbance
                - 1: synonym disturbance (rename action/tool names to synonyms)
                - 2: token disturbance (rename action/tool names to meaningless tokens like z1, z2, ...)
        """
        self.action_format = ActionFormat(action_format)
        self.disturb = int(disturb)
