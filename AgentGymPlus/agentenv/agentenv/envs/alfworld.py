...
class AlfWorldEnvClient(BaseEnvClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._disturb_mgr = DisturbManager(env_key="alfworld", disturb=getattr(self, "disturb", 0))
    ...
    def step(self, action: str) -> StepOutput:
        ...
        try:
            ...
            if self._disturb_mgr.enabled:
                translated, err = self._disturb_mgr.translate_prefix_command(action)
                if err is not None:
                    return StepOutput(
                        state=str(err) + "\n\n" + str(self.observe()),
                        reward=0.0,
            ...
            print(e, action)
            return StepOutput(
                state="Invalid Action.\n\n" + str(self.observe()), reward=0.0, done=False
            )
