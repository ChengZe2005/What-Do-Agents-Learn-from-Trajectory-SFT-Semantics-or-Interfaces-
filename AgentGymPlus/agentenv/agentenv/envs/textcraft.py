...
class TextCraftEnvClient(BaseEnvClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._disturb_mgr = DisturbManager(env_key="textcraft", disturb=getattr(self, "disturb", 0))
        ...
        self.env_id = ok["id"]
        if self._disturb_mgr.enabled:
            ok["observation"] = self._disturb_mgr.apply_to_any(ok["observation"])
        self.info = {
    ...
    def step(self, action: str) -> StepOutput:
        ...
        action_cmd = action_matches[-1] if action_matches else ""
        if self._disturb_mgr.enabled:
            translated, err = self._disturb_mgr.translate_prefix_command(action_cmd)
            if err is not None:
                return StepOutput(state=str(err), reward=0, done=False)
            action_cmd = translated

        action_cmd = re.sub(r"[^A-Za-z0-9, ]+", "", action_cmd)
        action_cmd = " ".join(action_cmd.split()).strip()
        response = self._post("step", {"action": action_cmd})
        self.info = {
            "observation": self._disturb_mgr.apply_to_any(response.get("observation")),
            "reward": response["reward"],
            "done": response["done"],
        }
        return StepOutput(
            state=str(self.info["observation"]),
            reward=response["reward"],
            done=response["done"],
        )
